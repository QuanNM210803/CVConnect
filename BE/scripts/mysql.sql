...
# alter table `cvconnect-user-service`.roles
# add column member_type varchar(255) not null after name;
# alter table `cvconnect-user-service`.menus
# add column sort_order int not null default 1 after parent_id;
...
# insert into `cvconnect-user-service`.menus(code, label, icon, url, parent_id, sort_order, created_by)
# values ('setup', 'Thiết lập', 'dashboard-icon', null, null, 1, 'admin')
# ('', 'Nhóm người dùng', '', );
